...
Page({
  ...
  getUserMsg() {
    ...
    var dateriqi
    //分页显示
   db.collection('diaocha').count().then((data)=>{
    ...
    db.collection('diaocha').orderBy('ctreated','desc').skip((pageindex - 1) * pagesize).limit(pagesize).get({
  ...
  delUserMsg(e) {
    ...
    db.collection('diaocha').doc(id).remove({
      success: function (res) {
        wx.showToast({
          title:"删除数据成功！"
        })
